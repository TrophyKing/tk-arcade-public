...
const privateKey = fs.readFileSync("./_.trophyking.app_private_key.key", "utf8");
const certificate = fs.readFileSync("./_.trophyking.app_ssl_certificate.cer", "utf8");
    ...
    rewardLedger = [
        {
            ...
            name: "KING",
            coin: KING_ADDRESS,
            price: getPrice(KING_ADDRESS) * KING_MINING_RATE,
            decimals: 18,
        },
    ];
...
const getItemByLobby = (lobby) => {
    switch (lobby) {
        ...
        case "king":
            return KING_INDEX[randomItem(0, KING_INDEX.length - 1)];
        case "tewken":
...
const synchronizeArcadeUsers = () => {
    ...
    arcadeUserList.forEach((arcadeUser) => {
        const arcadeRef = store.collection("chiharu_users").doc(arcadeUser.id);
                    ...
                    gameLobbies = produce(gameLobbies, (draft) => {
                        switch (active_powerup.item) {
                            ...
                            case 3: {
                                ...
                                const amount = Math.floor(factor / getPrice(KING_ADDRESS));
                                const coin = R.find(R.propEq("coin", KING_ADDRESS))(rewardLedger);
                                ...
                                console.log("KING reward", reward);
            ...
            if (room.id === id && room.count < 6) {
                ...
                if (arcadeUser) {
                    switch (lobby) {
                        default:
                            console.log(
                                "[INFO] Launching lobby competitive lobby for " + registrationFee[lobby] + " KING."
                            );
